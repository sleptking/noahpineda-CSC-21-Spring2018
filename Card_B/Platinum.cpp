...
Platinum::Platinum(){//Default Constructor
    set_number("0");
    set_firstname("none");
    set_lastname("none");
    set_type("none"); 
...
Platinum::Platinum(string Cnumber, string Cfirstname, string Clastname, string Ctype, double Cbalance){//Overloaded Constructor
    set_number(Cnumber);
...
Platinum::~Platinum(){//Destructor
    
}
//Member Functions
void Platinum::Rebate(){
    double addrebate = 0;
    addrebate = (get_total() * .03);
    set_total(get_total() + addrebate);
    set_rebate(addrebate);
}

void Platinum::RunTrans(){
    //Declare Variables
    ...
    while(count != itcount){
        ...
        if(get_type() == "Platinum"){
            if((get_balance() - cost) >= -1000){
